...
from src._agents.state import AgentState, setUpState, QuestionAskingState
...
def build_vector(state: setUpState) -> setUpState:
    """Build vector store"""
    ...
    try:
        ...
        print(e)
        is_completed = False
    return {'is_vector_completed': is_completed}


def build_bm25(state: setUpState) -> setUpState:
    """Build BM25 index"""
    print("[BM25] Building...")
    ...
    try:
        ...
        print(e)
        is_completed = False
    return {'is_BM25_completed': is_completed}


def build_graph(state: setUpState) -> setUpState:
    ...
    return {'is_graph_completed': is_completed}


def router_node(state: QuestionAskingState) -> QuestionAskingState:
...
def grader_node(state: QuestionAskingState) -> QuestionAskingState:
...
def expander_node(state: QuestionAskingState) -> QuestionAskingState:
...
def presenter_node(state: QuestionAskingState) -> QuestionAskingState:
...
def general_assistant_node(state: QuestionAskingState) -> QuestionAskingState:
...
def architecture_node(state: QuestionAskingState) -> QuestionAskingState:
    query = state.get('query', '')
    router_response = state.get('router_response', '')
