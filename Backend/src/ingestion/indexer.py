...
class SemanticIndexer(ast.NodeVisitor):
    def __init__(self, file_path, rel_path, global_class_registry):
        ...
        self.scope_stack = [self.module_name] #tracks current scope
       
        
        self.definitions = {} #stores all class/function metadata
        self.calls = [] #Stores all function call edges
        self.context_stack = [] 
        
        self.scope_aliases = defaultdict(dict) #tracks imports & aliases
         # {"src.utils.helper": {"np": "numpy", "pd": "pandas"}}
        
        self.scope_vars = defaultdict(dict) #Tracks variable → type mapping
         # {"src.utils.helper.calculate": {"user": "User", "score": "int"}}
         
        self.return_types = {}  #Stores function return types
          # {"src.utils.helper.get_user": "User"}
          
        self.current_class = None #tracks which class we are inside

    def _scope(self):
        """This functions help resolve current namespace."""
    ...
    def _find_alias(self, name):
        """if I write np.array, it resolves to numpy.array"""
    ...
    def visit_ImportFrom(self, node):
        # node.module = "src.models.user"
        # node.level = 0 (no dots)
    ...
    def visit_Assign(self, node):
        # node.targets = [Name(id='user')]
        # node.value = Call(func=Name(id='fetch_user_by_id'), args=[...])
